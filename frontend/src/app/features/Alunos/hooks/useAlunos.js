...
export function useAlunos() {
    ...
    const [busca, setBusca] = useState(() => localStorage.getItem('melvin_alunos_busca') || '');
    const [aula, setAula] = useState(() => localStorage.getItem('melvin_alunos_aula') || 'todos'); 
    const [filtroEspera, setFiltroEspera] = useState(() => localStorage.getItem('melvin_alunos_filtroEspera') === 'true');
    const [turnoSelecionado, setTurnoSelecionado] = useState(() => localStorage.getItem('melvin_alunos_turnoSelecionado') || 'todos');

    useEffect(() => {
        localStorage.setItem('melvin_alunos_busca', busca);
    }, [busca]);

    useEffect(() => {
        localStorage.setItem('melvin_alunos_aula', aula);
    }, [aula]);

    useEffect(() => {
        localStorage.setItem('melvin_alunos_filtroEspera', filtroEspera);
    }, [filtroEspera]);

    useEffect(() => {
        localStorage.setItem('melvin_alunos_turnoSelecionado', turnoSelecionado);
    }, [turnoSelecionado]);
