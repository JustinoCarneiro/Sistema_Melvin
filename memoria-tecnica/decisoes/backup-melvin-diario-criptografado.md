...
- **⚠️ Risco de expiração do token em 7 dias:** o app OAuth criado no Google Cloud (`rclone-melvin-backup-504515`) está com o status de publicação em **"Testando"**. Apps OAuth do Google nesse status costumam ter o refresh token expirado após 7 dias de inatividade/teste, o que quebraria o upload off-site silenciosamente (o backup local continua funcionando, só o envio pro Drive falharia, com alerta por e-mail avisando). **Ação pendente:** publicar o app em produção (Google Cloud Console → Público-alvo → Publish App) — como o escopo usado é só `drive.file` (não sensível), não deve exigir verificação do Google. Verificar isso antes do dia 11/08/2026.
